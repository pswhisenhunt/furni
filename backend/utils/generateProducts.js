...
const generateProducts = async () => {
  ...
  const adjectives = ['wooden','large','metal','silky','small','medium','rod iron','king sized','queen sized','twin sized','cozy','modern','boho']

  const products = []
  for (let i = 0; i < 100; i++) {
    let type = types[getRandomInt(types.length - 1)]
    products.push({
      number: `000${getRandomInt(1000)}`,
      type: type,
      ...
      price: Math.floor(Math.random() * (10 * 100 - 1 * 100) + 1 * 100) / (1/100)
    })
  }
